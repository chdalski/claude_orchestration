...
Security Engineer reviews throughout. For non-code tasks
(documentation, prose), the Test Engineer sends "no tests
needed" instead of "tests ready" and the Security Engineer
confirms "no security implications" — the Developer
proceeds after both signals. Security Engineer sends
post-implementation sign-off to the dev-team. Dev-team
reports completion to the lead only after receiving the
security sign-off.
...
- **All tests in one batch** — the Test Engineer writes
  ...
  splitting creates stop-start cycles where the Developer
  partially implements, waits again, and risks
  inconsistent design.
