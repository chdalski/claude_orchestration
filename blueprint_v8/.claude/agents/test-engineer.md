...
4. For integration tests: before choosing a test harness
   approach, study how the framework itself tests similar
...
- Write **all** tests up front in a single batch — unit
  ...
  writing into phases.
- **Exception: spike integration tests first.** Before
  writing the full set of integration tests, write and
  run one integration test to confirm the test harness
  works. If the spike fails due to framework behavior,
  fix the harness before writing the rest. This does
  not delay the "tests ready" message — just do the
  spike as part of the batch.
...
- Send one "tests ready" message when all test files are
  delivered. The Developer waits for this message. For
  non-code tasks, send "no tests needed" instead.
