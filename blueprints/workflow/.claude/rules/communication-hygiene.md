...
**Exception: solicited handoff receipts.** When a dispatch
message instructs you to acknowledge receipt — because the
workflow's handoff-monitoring protocol uses explicit acks
to detect silent message drops at peer-to-peer transitions
— the ack is a protocol receipt, not noise. It is
solicited; the sender is actively waiting for it. Send a
brief acknowledgment as instructed (e.g., "received,
starting <next step>"). This exception applies only to
acks the dispatch explicitly requests — it does not
license unsolicited acknowledgments elsewhere.
