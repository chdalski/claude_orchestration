...
| `post-start.sh` | Every start: Claude config, git identity, plugins |
...
`.env.defaults` sets `proxy`; override it in `.devcontainer/.env`.
...
TypeScript and Python plugins; `rust-analyzer` and `gopls` come with the Rust
and Go features.
