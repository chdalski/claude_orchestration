...
LOG_PATH = os.getenv("PROXY_LOG", "/workspace/cache-proxy.log")
...
def log(message: str, prefix: str = "INFO"):
    ...
    print(f"[{timestamp}] {prefix}: {message}", file=LOG_FILE)
...
class PromptCachingAddon:
    ...
    def request(self, flow: http.HTTPFlow) -> None:
        ...
        if "messages" not in flow.request.path:
            log(f"  - Skipping (not a messages endpoint)")
            return
...
def start_proxy():
    ...
    sys.argv = [
        ...
        str(port),
        "--set",
        f"connection_strategy=lazy",
        "--set",
        f"stream_large_bodies=1m",
        "--set",
        "websocket=false",
        "--ssl-insecure",  # Don't verify upstream SSL (for development)
        "-s",
        __file__,  # Load this file as addon
    ]
