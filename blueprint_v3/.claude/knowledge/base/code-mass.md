...
function validate(request):
    errors = []                  // binding(1) + invocation(2)

    if request.id < 0:           // conditional(4)
                                 //   + invocation(2) + constant(1)
        errors.push(NegativeId)  // assignment(6) + invocation(2)

    if request.items.isEmpty():  // conditional(4) + invocation(2)
        errors.push(NoItems)     // assignment(6) + invocation(2)

    if errors.isEmpty():         // conditional(4) + invocation(2)
        return Ok()              // invocation(2)
    else:
        return Err(errors)       // invocation(2) + binding(1)
```
