...
The architecture enables targeted testing at each layer
(see `testing.md` for the testing pyramid):

- **Domain (unit tests)** — test business logic directly,
  no mocks needed because the domain has no dependencies
- **Inbound adapters (integration tests)** — test handlers
  with mock implementations of domain services
- **Outbound adapters (integration tests)** — test
  repository implementations against real infrastructure
  (test containers, in-memory databases)
- **End-to-end** — test the full stack for critical paths
  only

Because ports are defined as interfaces/traits, test
doubles are straightforward — implement the port with
test data, no mocking library required.
